...
import cn.lezoo.doux.framework.conn.wrapper.pool.HikariPoolMXBean;
...
import cn.lezoo.doux.framework.conn.wrapper.pool.metrics.dropwizard.CodahaleHealthChecker;
import cn.lezoo.doux.framework.conn.wrapper.pool.metrics.dropwizard.CodahaleMetricsTrackerFactory;
import cn.lezoo.doux.framework.conn.wrapper.pool.metrics.micrometer.MicrometerMetricsTrackerFactory;
import cn.lezoo.doux.framework.conn.wrapper.pool.util.ConcurrentBag;
import cn.lezoo.doux.framework.conn.wrapper.pool.util.ConcurrentBag.IBagStateListener;
import cn.lezoo.doux.framework.conn.wrapper.pool.util.SuspendResumeLock;
...
import static cn.lezoo.doux.framework.conn.wrapper.pool.util.ClockSource.plusMillis;
import static cn.lezoo.doux.framework.conn.wrapper.pool.util.ConcurrentBag.IConcurrentBagEntry.STATE_IN_USE;
import static cn.lezoo.doux.framework.conn.wrapper.pool.util.ConcurrentBag.IConcurrentBagEntry.STATE_NOT_IN_USE;
import static cn.lezoo.doux.framework.conn.wrapper.pool.util.UtilityElf.CustomDiscardPolicy;
import static cn.lezoo.doux.framework.conn.wrapper.pool.util.UtilityElf.DefaultThreadFactory;
import static cn.lezoo.doux.framework.conn.wrapper.pool.util.UtilityElf.createThreadPoolExecutor;
import static cn.lezoo.doux.framework.conn.wrapper.pool.util.UtilityElf.quietlySleep;
...
public final class HikariPool extends PoolBase implements HikariPoolMXBean, IBagStateListener {
    ...
    private final long aliveBypassWindowMs = Long.getLong(".aliveBypassWindowMs", MILLISECONDS.toMillis(500));
    private final long housekeepingPeriodMs = Long.getLong("cn.lezoo.doux.framework.conn.wrapper.pool.housekeeping.periodMs", SECONDS.toMillis(30));
    ...
    public HikariPool(final HikariConfig config) {
        ...
        if (Boolean.getBoolean("cn.lezoo.doux.framework.conn.wrapper.pool.blockUntilFilled") && config.getInitializationFailTimeout() > 1) {
     ...
     * @return a java.sql.Connection instance
     */
    public Connection getConnection(final long hardTimeout) {
        ...
        try {
            ...
            throw new PoolException(poolName + " - Interrupted during connection acquisition", e);
        } finally {
    ...
    public synchronized void shutdown() throws InterruptedException {
        try {
            ...
            if (!addConnectionExecutor.awaitTermination(5, SECONDS)) {
                log.warn("Timed-out waiting for add connection executor to shutdown");
            }
            ...
            // finally {
                ...
                //     log.warn("Timed-out waiting for connection assassin to shutdown");
                // }
            // }

            closeConnectionExecutor.shutdown();
            if (!closeConnectionExecutor.awaitTermination(10L, SECONDS)) {
                log.warn("Timed-out waiting for close connection executor to shutdown");
            }
        } finally {
    ...
    private PoolEntry createPoolEntry() {
        try {
            ...
            if (maxLifetime > 0) {
                ...
                poolEntry.setEndOfLife(houseKeepingExecutorService.schedule(new MaxLifetimeTask(poolEntry), lifetime, MILLISECONDS));
            }
            ...
            if (poolState == POOL_NORMAL) { // we check POOL_NORMAL to avoid a flood of messages if shutdown() is running concurrently
                log.error("{} - Error thrown while acquiring connection from data source", poolName, e.getCause());
                lastConnectionFailure.set(e);
            }
        } catch (Exception e) {
            if (poolState == POOL_NORMAL) { // we check POOL_NORMAL to avoid a flood of messages if shutdown() is running concurrently
                log.debug("{} - Cannot acquire connection from data source", poolName, e);
            }
        }
    ...
    private synchronized void fillPool(final boolean isAfterAdd) {
        final int idle = getIdleConnections();
        final boolean shouldAdd = getTotalConnections() < config.getMaximumPoolSize() && idle < config.getMinimumIdle();

        if (shouldAdd) {
            final int countToAdd = config.getMinimumIdle() - idle;
            for (int i = 0; i < countToAdd; i++)
                addConnectionExecutor.submit(isAfterAdd ? postFillPoolEntryCreator : poolEntryCreator);
        } else if (isAfterAdd) {
            log.debug("{} - Fill pool skipped, pool has sufficient level or currently being filled.", poolName);
        }
    }
        ...
        do {
            ...
            if (poolEntry != null) {
                if (config.getMinimumIdle() > 0) {
                    ...
                    log.info("{} - Added connection {}", poolName, poolEntry.connection);
                } else {
     ...
     * through {@link cn.lezoo.doux.framework.conn.wrapper.pool.HikariDataSource#evictConnection(Connection)} then {@code owner} is {@code true}.
     * <p>
    ...
    private ScheduledExecutorService initializeHouseKeepingExecutorService() {
        if (config.getScheduledExecutor() == null) {
            final ThreadFactory threadFactory = Optional.ofNullable(config.getThreadFactory()).orElseGet(() -> new DefaultThreadFactory(poolName + " housekeeper"));
    ...
    /**
     * Create a timeout exception to be thrown, because a
     ...
     */
    private PoolException createTimeoutException(long startTime) {
                ...
                while (shouldContinueCreating()) {
                    ...
                    if (poolEntry != null) {
                        ...
                        if (loggingPrefix != null && backoffMs % 50 == 0)
                            log.debug("{} - Connection add failed, sleeping with backoff: {}ms", poolName, backoffMs);
        ...
        public void run() {
            try {
                ...
                if (plusMillis(now, 128) < plusMillis(previous, housekeepingPeriodMs)) {
                    ...
                    log.warn("{} - Thread starvation or clock leap detected (housekeeper delta={}).", poolName, elapsedDisplayString(previous, now));
                }
                ...
                if (idleTimeout > 0L && config.getMinimumIdle() < config.getMaximumPoolSize()) {
                    ...
                    int maxToRemove = notInUse.size() - config.getMinimumIdle();
